...
echo "Scraper elbordo.pl (użytkownik tigrisek1) started.\n";

$is_forced = !empty($_GET['force']) || (isset($argv) && in_array('force=1', $argv));

// Check if scraper is enabled in settings
$scraper_enabled = isset($settings['scraper_mylomza_enabled']) ? (bool)$settings['scraper_mylomza_enabled'] : false;
if (!$scraper_enabled && !$is_forced) {
	echo "Scraper (myLomza) is disabled in settings.\n";
	exit(0);
}

// Pseudo-cron Lock & Rate Limiting (InfinityFree support)
$now = time();
$last_run = isset($settings['cron_last_scraper_mylomza']) ? (int)$settings['cron_last_scraper_mylomza'] : 0;
if (php_sapi_name() !== 'cli' && !$is_forced && ($now - $last_run) < 300) {
	echo "Scraper was already executed recently (lock active). Exiting.\n";
	exit(0);
}
$db->query("UPDATE `"._DB_PREFIX_."settings` SET value = UNIX_TIMESTAMP() WHERE name = 'cron_last_scraper_mylomza'");

$display_days = isset($settings['scraper_mylomza_display_days']) ? (int)$settings['scraper_mylomza_display_days'] : 7;
if ($display_days <= 0) {
	$display_days = 7;
}
echo "Configured display days: $display_days\n";

$max_imports = isset($settings['scraper_mylomza_max_imports']) ? (int)$settings['scraper_mylomza_max_imports'] : 30;
if ($max_imports <= 0) {
	$max_imports = 30;
}
echo "Configured maximum imports per run: $max_imports\n";

$user_profile_url = "https://elbordo.pl/ogloszenia/uzytkownik/7178/tigrisek1/";
...
function resolveAntykiCategoryIdForTigrisek($title, $description) {
	global $db;
...
function processOfferPhotosForTigrisek($photos, $offer_id) {
	global $db, $settings;
...
foreach ($links as $itemUrl) {
	...
	if (preg_match('/<script id="exportDatavEd1" type="application\/json">(.*?)<\/script>/s', $itemHtml, $matches)) {
		...
		if ($jsonData && isset($jsonData['details'])) {
			...
			// Clean up description HTML or trailing placeholder
			$description = preg_replace('/telefon\s*\*+$/ui', '', $rawText);
			$description = trim(strip_tags($description, '<p><br><strong><b><i><em><ul><li>'));
	...
	$formattedDescription = '<p>' . nl2br($description) . '</p>';
	...
	$category_id = resolveAntykiCategoryIdForTigrisek($title, $description);
	...
	if (!empty($photos) && $offer_id > 0) {
		processOfferPhotosForTigrisek($photos, $offer_id);
	}
