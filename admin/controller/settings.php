...
if($admin->is_logged()){
	...
	if(!_ADMIN_TEST_MODE_ and isset($_POST['action'])){
		if($_POST['action']=='save_settings' and !empty($_POST['base_url']) and !empty($_POST['email']) and !empty($_POST['title']) and checkToken('admin_save_settings')){
			...
			$sth->bindValue(':value', isset($_POST['enable_verification_badges']), PDO::PARAM_INT);
			...
			$sth->bindValue(':value', isset($_POST['scraper_enabled']), PDO::PARAM_INT);
			$sth->bindValue(':name', 'scraper_enabled', PDO::PARAM_STR);
			$sth->execute();

			$sth->bindValue(':value', (int)($_POST['scraper_display_days'] ?? 7), PDO::PARAM_INT);
			$sth->bindValue(':name', 'scraper_display_days', PDO::PARAM_STR);
			$sth->execute();

			$sth->bindValue(':value', (int)($_POST['scraper_max_imports'] ?? 30), PDO::PARAM_INT);
			$sth->bindValue(':name', 'scraper_max_imports', PDO::PARAM_STR);
			$sth->execute();

			$sth->bindValue(':value', isset($_POST['scraper_mylomza_enabled']), PDO::PARAM_INT);
			$sth->bindValue(':name', 'scraper_mylomza_enabled', PDO::PARAM_STR);
			$sth->execute();

			$sth->bindValue(':value', (int)($_POST['scraper_mylomza_display_days'] ?? 7), PDO::PARAM_INT);
			$sth->bindValue(':name', 'scraper_mylomza_display_days', PDO::PARAM_STR);
			$sth->execute();

			$sth->bindValue(':value', (int)($_POST['scraper_mylomza_max_imports'] ?? 10), PDO::PARAM_INT);
			$sth->bindValue(':name', 'scraper_mylomza_max_imports', PDO::PARAM_STR);
			$sth->execute();

			$sth->bindValue(':value', isset($_POST['scraper_eostroleka_enabled']), PDO::PARAM_INT);
			$sth->bindValue(':name', 'scraper_eostroleka_enabled', PDO::PARAM_STR);
			$sth->execute();

			$sth->bindValue(':value', (int)($_POST['scraper_eostroleka_display_days'] ?? 7), PDO::PARAM_INT);
			$sth->bindValue(':name', 'scraper_eostroleka_display_days', PDO::PARAM_STR);
			$sth->execute();

			$sth->bindValue(':value', (int)($_POST['scraper_eostroleka_max_imports'] ?? 10), PDO::PARAM_INT);
			$sth->bindValue(':name', 'scraper_eostroleka_max_imports', PDO::PARAM_STR);
			$sth->execute();

			$sth->bindValue(':value', isset($_POST['scraper_mojaostroleka_enabled']), PDO::PARAM_INT);
			$sth->bindValue(':name', 'scraper_mojaostroleka_enabled', PDO::PARAM_STR);
			$sth->execute();

			$sth->bindValue(':value', (int)($_POST['scraper_mojaostroleka_display_days'] ?? 7), PDO::PARAM_INT);
			$sth->bindValue(':name', 'scraper_mojaostroleka_display_days', PDO::PARAM_STR);
			$sth->execute();

			$sth->bindValue(':value', (int)($_POST['scraper_mojaostroleka_max_imports'] ?? 10), PDO::PARAM_INT);
