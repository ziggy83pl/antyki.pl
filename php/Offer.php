...
class Offer {
     ...
     * @return mixed[]
     */
    public static function loadOffers(int $limit=10,$type='all'): array{
		global $user;
        $db = \App\Core\App::db();
		...
		if(isset($_GET['search']) and !empty($_GET['address'])){
			if (isset($_GET['distance'])) {
				$distance = intval($_GET['distance']);
			} else {
				if (!empty($_GET['state']) && !empty($_GET['state2'])) {
					$distance = 10;
				}
			}
		...
		if(!$use_distance_search && !empty($_GET['state'])){
		...
		if(isset($_GET['search'])){
			...
			if(!empty($_GET['name'])){
				$names = explode(' ', (string) $_GET['name']);
				$where_statement .= ' AND ( ';
			...
			if(!empty($_GET['keywords'])){
				if(isset($_GET['exact_phrase'])){
					...
					$keywords = explode(' ', (string) $_GET['keywords']);
					$where_statement .= ' AND ( ';
			...
			if($use_distance_search){
				...
				$bind_values['distance'] = $distance;
			}elseif(!empty($_GET['address']) && empty($_GET['state'])){
		...
		if($type=='my_offers'){
			...
			$select .= ', (SELECT count(1) FROM '._DB_PREFIX_.'logs_offer WHERE offer_id=o.id) AS view_all, (SELECT count(Distinct lo.ip) FROM '._DB_PREFIX_.'logs_offer lo WHERE offer_id=o.id) AS view_unique ';
		...
		if($type=='offers' or $type=='my_offers' or $type=='clipboard' or $type=='index_page'){
		...
		// --- 1. ZAPYTANIE LICZĄCE (Zamiast SQL_CALC_FOUND_ROWS) ---
		if ($use_distance_search) {
			$countSql = 'SELECT COUNT(1) FROM (SELECT o.id ' . $select . ' FROM ' . _DB_PREFIX_ . 'offer o ' . $join . ' WHERE ' . $where_statement . ') as sub';
		} else {
			$countSql = 'SELECT COUNT(o.id) FROM ' . _DB_PREFIX_ . 'offer o ' . $join . ' WHERE ' . $where_statement;
		}
		
		$countSth = $db->prepare($countSql);
		foreach($bind_values as $key => $value){
			$countSth->bindValue(':'.$key, $value, PDO::PARAM_STR);
		}
		$countSth->execute();
		$totalOffers = (int)$countSth->fetchColumn();

		// --- 2. GŁÓWNE ZAPYTANIE (Bez SQL_CALC_FOUND_ROWS) ---
		$offers = [];

		$sth = $db->prepare('SELECT o.* '.$select.', 
		...
		while($row = $sth->fetch(PDO::FETCH_ASSOC)){
			if($type=='my_offers' and $settings['allow_refresh_offer']){
				...
				$row['refresh']['days'] = ceil((strtotime((string) $row['date_finish']) - time())/(60*60*24)-$settings['days_before_refresh']);
		...
		generatePagination($limit, $totalOffers);

		return $offers;
	}

	public static function addOffer(array $data): array{
		global $user;
        $db = \App\Core\App::db();
        $settings = \App\Core\App::settings();
	...
	public static function editOffer(int $id,array $data,$checkCost=false): array{
		global $user;
        $db = \App\Core\App::db();
        $settings = \App\Core\App::settings();
		...
		if(!empty($data['address'])){$data['address'] = trim(strip_tags((string) $data['address']));}else{$data['address'] = null;}
		$data['category_id'] = isset($data['category_id']) ? (int)$data['category_id'] : 0;
		$data['state_id'] = isset($data['state_id']) ? (int)$data['state_id'] : 0;
		$data['state2_id'] = isset($data['state2_id']) ? (int)$data['state2_id'] : 0;
		$data['type_id'] = isset($data['type_id']) ? (int)$data['type_id'] : 0;
		...
		$sth = $db->prepare('UPDATE '._DB_PREFIX_.'offer SET name=:name, slug=:slug, address=:address, phone=:phone, email=:email, category_id=:category_id, state_id=:state_id, state2_id=:state2_id, type_id=:type_id, description=:description, address_lat=:address_lat, address_long=:address_long, price=:price, price_negotiate=:price_negotiate, price_free=:price_free WHERE id=:id LIMIT 1');
		$name = substr(trim((string) checkWordsBlackList(strip_tags((string) $data['name']))),0,$settings['number_char_title']);
		$slug = slug($name);
		$sth->bindValue(':id', $id, PDO::PARAM_INT);
		...
		$sth->bindValue(':email', trim(strip_tags((string) $data['email'])), PDO::PARAM_STR);
		...
		$sth->bindValue(':description', checkWordsBlackList(nofollow($purifier->purify(trim((string) $data['description'])))), PDO::PARAM_STR);
		...
		if($settings['photo_add'] and !empty($data['photos'])){
			...
			if($user->getId() and $user->moderator){
				...
				$sth = $db->prepare('SELECT * from '._DB_PREFIX_.'photo WHERE id=:id AND (user_id=:user_id OR (offer_id=:offer_id AND offer_id>0)) LIMIT 1');
				$sth->bindValue(':user_id', $user->getId(), PDO::PARAM_INT);
				$sth->bindValue(':offer_id', $id, PDO::PARAM_INT);
			}
			$photo_position = 0;
			...
			foreach($data['options'] as $key=>$value){
				if(is_array($value)){
					foreach($value as $value2){
						...
						$sth->bindValue(':value', trim(strip_tags((string) $value2)), PDO::PARAM_STR);
						$sth->execute();
					}
				}elseif($value){
					$sth->bindValue(':option_id', $key, PDO::PARAM_INT);
					$sth->bindValue(':value', trim(strip_tags((string) $value)), PDO::PARAM_STR);
					$sth->execute();
				}
			}
		}
	...
	public static function loadOffer(int $id,$controller=''){
		...
		$select = '';
		if($controller == 'offer'){
			$select .= ', c.name as category_name, c.slug as category_slug, s.name as state_name, s.slug as state_slug, s2.name as state2_name, s2.slug as state2_slug, t.name as type_name, t.slug as type_slug, u.username as username, u.avatar as user_avatar, u.url_website as user_url_website, u.company_name as user_company_name, u.verified_email as user_verified_email, u.verified_phone as user_verified_phone, u.verified_company as user_verified_company, (SELECT count(1) FROM '._DB_PREFIX_.'logs_offer WHERE offer_id=o.id) AS view_all, (SELECT count(Distinct lo.ip) FROM '._DB_PREFIX_.'logs_offer lo WHERE offer_id=o.id) AS view_unique, (SELECT AVG(rating) FROM '._DB_PREFIX_.'opinion WHERE user_id=o.user_id AND active=1) AS user_rating_avg, (SELECT COUNT(id) FROM '._DB_PREFIX_.'opinion WHERE user_id=o.user_id AND active=1) AS user_rating_count, (SELECT COUNT(id) FROM '._DB_PREFIX_.'opinion WHERE user_id=o.user_id AND active=1 AND rating>=4) AS user_rating_positive_count ';
			$join .= 'LEFT JOIN '._DB_PREFIX_.'category c ON o.category_id = c.id
			LEFT JOIN '._DB_PREFIX_.'state s ON o.state_id = s.id
			LEFT JOIN '._DB_PREFIX_.'state s2 ON o.state2_id = s2.id
			LEFT JOIN '._DB_PREFIX_.'type t ON o.type_id = t.id
		...
		if($offer){
			...
			if($controller=='edit' || $controller=='add_similar'){
				...
				$category = Category::getCategory((int)$offer['category_id'], true);
				$offer['categories'] = [];
				...
				$excluded_ips = [];
				if(!empty($settings['exclude_ip_views'])){
					$decoded = json_decode((string) $settings['exclude_ip_views'], true);
					if(is_array($decoded)){
						$excluded_ips = array_keys($decoded);
					}else{
						$excluded_ips = array_map(trim(...), explode(PHP_EOL, (string) $settings['exclude_ip_views']));
					}
				}
				if(!in_array($client_ip, $excluded_ips)){
					...
					$sth->bindValue(':offer_id', $id, PDO::PARAM_INT);
					$sth->bindValue(':user_id', $user->getId(), PDO::PARAM_INT);
					$sth->bindValue(':ip', $client_ip, PDO::PARAM_STR);
					$sth->execute();
				}
	...
	public static function checkActiveOffer(int $id,$code=''): bool{
		global $user;
        $db = \App\Core\App::db();
		if($user->moderator){
	...
	public static function activateCode($code): void{
		global $user;
        $db = \App\Core\App::db();
        $settings = \App\Core\App::settings();
		...
		if($offer and static::countCost($offer['id'])['total']==0){
			$user_id = \App\User::getIdFromEmail($offer['email']);
	...
	public static function checkPermissions(int $id,$code=''): bool{
		global $user;
        $db = \App\Core\App::db();
		
		// Bypass permissions for active administrator panel session
		if (isset($_SESSION['admin']['id']) && isset($_SESSION['admin']['session_code'])) {
			$sthAdmin = $db->prepare('SELECT 1 FROM '._DB_PREFIX_.'admin_session WHERE user_id=:user_id AND code=:code LIMIT 1');
			$sthAdmin->bindValue(':user_id', $_SESSION['admin']['id'], PDO::PARAM_INT);
			$sthAdmin->bindValue(':code', $_SESSION['admin']['session_code'], PDO::PARAM_STR);
			$sthAdmin->execute();
			if ($sthAdmin->fetchColumn()) {
				return true;
			}
		}
	...
	public static function checkSessionCode($session_code): bool{
		$db = \App\Core\App::db();
        $settings = \App\Core\App::settings();
	...
	public static function getViews(int $id): array{
		$db = \App\Core\App::db();
	...
	public static function activate(int $id,$date_finish='',$admin_confirmed=0): void{
		$db = \App\Core\App::db();
        $settings = \App\Core\App::settings();
		if(!$date_finish){
	...
	public static function disablePromote(int $id): void{
		$db = \App\Core\App::db();
	...
	public static function enablePromote(int $id,$date=false): void{
		$db = \App\Core\App::db();
        $settings = \App\Core\App::settings();
		$offer = static::loadOffer($id);
		$date_finish = date("Y-m-d", strtotime((string) $offer['date_finish']));
		if($date){
	...
	public static function remove(int $id): void{
		...
		if($offer){
			...
			if($offer['category_id']){
				Category::refreshCount((int)$offer['category_id']);
			}
		}
	}

	public static function refreshCountCategories(int $id): void{
		$offer = static::loadOffer($id);
		if($offer['category_id']){
			Category::refreshCount((int)$offer['category_id']);
		}
     ...
     * @return mixed[]
     */
    public static function loadSimilarOffers(array $offer, int $limit=10): array{
		$db = \App\Core\App::db();

		$where_statement = ' (false ';

		if($offer['category_id']){
			$where_statement .= ' OR o.category_id = '.$offer['category_id'];
		}
		if($offer['state_id']){
			$where_statement .= ' OR o.state_id = '.$offer['state_id'];
		}
		if($offer['type_id']){
			$where_statement .= ' OR o.type_id = '.$offer['type_id'];
		}

		$where_statement .= ' ) ';

		$similar_offers = [];
		$sth = $db->prepare('SELECT o.*, c.name as category_name, c.slug as category_slug, (SELECT CONCAT(folder,thumb) FROM '._DB_PREFIX_.'photo WHERE offer_id=o.id LIMIT 1) AS thumb
			FROM '._DB_PREFIX_.'offer o LEFT JOIN '._DB_PREFIX_.'category c ON o.category_id = c.id WHERE o.id!=:id AND o.active=1 AND '.$where_statement.' ORDER BY rand() LIMIT :limit_to');
		$sth->bindValue(':id', $offer['id'], PDO::PARAM_INT);
		$sth->bindValue(':limit_to', $limit, PDO::PARAM_INT);
		$sth->execute();
		while ($row = $sth->fetch(PDO::FETCH_ASSOC)){
	...
	public static function clipboardAdd(int $offer_id): void{
		global $user;
        $db = \App\Core\App::db();
	...
	public static function clipboardRemove(int $offer_id): void{
		global $user;
        $db = \App\Core\App::db();
     ...
     * @return mixed[]
     */
    public static function addPhoto(): array{
		global $user;
        $db = \App\Core\App::db();
        $settings = \App\Core\App::settings();

		$path_parts = pathinfo((string) $_FILES['file']['name']);
		...
		if($settings['photo_max_size'] and $_FILES["file"]["size"] > $settings['photo_max_size']*1024){
			...
			$url = substr((string) slug($path_parts['filename']), 0, 200).'.'.$path_parts['extension'];
			$i = 0;
			while(file_exists(_FOLDER_PHOTOS_.$folder.$url)) {
				$url = substr((string) slug($path_parts['filename']), 0, 200).'_'.$i.'.'.$path_parts['extension'];
				$i++;
			}
			...
			if (!$src) {
				return ['status'=>0, 'info'=>lang('Failed to create image from source')];
			}

			if(!empty($settings['watermark_add'])){
				$stamp_parts = pathinfo((string) $settings['watermark']);
				$stamp_parts['extension'] = strtolower($stamp_parts['extension']);

				if(in_array($stamp_parts['extension'] , ['jpg','jpeg','gif','png'])){
					// Próbujemy załadować znak wodny z lokalnej ścieżki zamiast URL
					$watermark_path = _FOLDER_PHOTOS_ . '../images/' . basename((string) $settings['watermark']);
					if($stamp_parts['extension']=="jpg" || $stamp_parts['extension']=="jpeg"){
						...
						$stamp = @imagecreatefromgif($watermark_path);
					}
					if($stamp){
				...
				if($path_parts['extension']=="jpg" || $path_parts['extension']=="jpeg"){
					...
					imagewebp($src,_FOLDER_PHOTOS_.$folder.$url,$settings['photo_quality']);
				}
			}else{
				move_uploaded_file($_FILES['file']['tmp_name'], _FOLDER_PHOTOS_.$folder.$url);
			}

			[$width, $height]=getimagesize(_FOLDER_PHOTOS_.$folder.$url);
			...
			if($settings['photo_max_height'] or $settings['photo_max_width']){
				...
				if($path_parts['extension']=="jpg" || $path_parts['extension']=="jpeg"){
					...
					imagewebp($new_image,_FOLDER_PHOTOS_.$folder.$url,$settings['photo_quality']);
				}
			...
			if($path_parts['extension']=="jpg" || $path_parts['extension']=="jpeg"){
				...
				imagewebp($tmp,_FOLDER_PHOTOS_.$folder.$thumb,100);
			}
			imagedestroy($src);
     ...
     * @return mixed[]
     */
    public static function countCost(int $id): array{
		$db = \App\Core\App::db();
        $settings = \App\Core\App::settings();
		$costs = [];
		$offer = static::loadOffer($id);
		if($offer){
			...
			$category = Category::getCategory((int)$offer['category_id']);
			if($category){
     ...
     * @return mixed[]
     */
    public static function getNames($name): array{
		$db = \App\Core\App::db();
		$names = [];
