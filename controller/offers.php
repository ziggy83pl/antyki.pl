...
if(_OFFERS_PATH_ and (isset($_GET['state']) or isset($_GET['category']) or isset($_GET['type']))){
	...
	if(isset($_GET['category'])){
		if($_GET['category']>0){
			$category_data = \App\Category::getCategory((int)$_GET['category']);
			if($category_data){
...
if(!empty($_GET['category_path'])){
	$category = \App\Category::getCategoryFromPath($_GET['category_path']);
	if($category){
...
if(isset($_GET['category']) and $_GET['category']>0){
	$categories = \App\Category::getCategories((int)$_GET['category']);
	if(empty($category)){
		$category = \App\Category::getCategory((int)$_GET['category']);
	}
}else{
...
if($settings['show_breadcrumbs']){
	...
	$path_temp = $path;
	if(!empty($category)){
		foreach(\App\Category::getBreadcrumbs($category) as $key=>$item){
...
if(!empty($category)){
	if(empty($categories)){
		$categories = \App\Category::getCategories((int)$category['category_id']);
	}
...
$render_variables['offers'] = \App\Offer::loadOffers($settings['limit_page'],'offers');
...
if(!empty($categories)){
	...
	if(isset($category)){
		$overCategory = \App\Category::getCategory((int)$category['category_id']);
		if($overCategory){
...
if($settings['search_box_options']){
	$render_variables['options'] = \App\Option::getOptions($category_id,'search');
}
if($settings['search_box_price']){
  $render_variables['search_show_price'] = \App\Option::checkShowPrice($category_id);
}else{
  $render_variables['search_show_price'] = false;
}
$render_variables['all_categories'] = \App\Category::getAllCategories();
